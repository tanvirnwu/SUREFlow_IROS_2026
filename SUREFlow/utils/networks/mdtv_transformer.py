...
class MDTVTransformer(nn.Module):
    ...
    def __init__(
        ...
        block_size = lang_tok_len + action_seq_len + perception_seq_len * self.n_obs_token + 2
        self.action_seq_len = action_seq_len
        self.use_modality_encoder = use_modality_encoder
        seq_size = lang_tok_len + perception_seq_len * self.n_obs_token + action_seq_len
        print(f"obs dim: {obs_dim}, lang_emb_dim: {lang_emb_dim}, action_dim: {action_dim}, proprio_dim: {proprio_dim}")
    ...
    def forward_enc_only(self, states, actions=None, goals=None, sigma=None, uncond: Optional[bool] = False):
        ...
        if not self.use_ada_conditioning:
            state_embed = torch.cat([emb_t, state_embed])

        context = self.encoder(state_embed)

        # state_embed, proprio_embed = self.process_state_embeddings(states)
        # lang_embed = self.process_goal_embeddings(goals, states)
        # input_seq = self.concatenate_inputs(emb_t, lang_embed, state_embed, proprio_embed, uncond)
    ...
    def apply_position_embeddings(self, lang_embed, state_embed, action_embed, proprio_embed, t):
        pos_len = t + self.lang_tok_len + self.action_seq_len - 1
        ...
        action_x = self.drop(action_embed + position_embeddings[:, (self.lang_tok_len + t - 1):, :])
        proprio_x = self.drop(proprio_embed + position_embeddings[:, self.lang_tok_len:(self.lang_tok_len + t), :]) if proprio_embed is not None else None
