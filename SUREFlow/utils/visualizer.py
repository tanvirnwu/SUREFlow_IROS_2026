...
class Visualizer():
    ...
    def _get_traversal_range(self, mean=0, std=1):
        ...
        if max_traversal < 0.5:
            ...
            max_traversal = stats.norm.ppf(max_traversal, loc=mean.cpu(), scale=std.cpu())  # from 0.05 to -1.645

        # symmetrical traversals
        return (-5 * max_traversal, 5 * max_traversal)
    ...
    def _traverse_line(self, idx, n_samples, data=None):
        ...
        if data is None:
            ...
            with torch.no_grad():
                samples, post_mean, post_logvar = self.model.encode(data.to(self.device))
                # samples = self.model.reparameterize(post_mean, post_logvar)
                samples = samples.to(self.device).repeat(n_samples, 1)
                post_mean_idx = post_mean.to(self.device)[0, idx]
                post_std_idx = torch.exp(post_logvar / 2).to(self.device)[0, idx]

            # travers from the gaussian of the posterior in case quantile
            traversals = torch.linspace(*self._get_traversal_range(mean=post_mean_idx, std=post_std_idx), steps=n_samples)
    ...
    def _save_or_return(self, to_plot, size, filename, is_force_return=False):
        ...
        kwargs = dict(nrow=size[1], pad_value=0.5)
    ...
    def _decode_latents(self, latent_samples):
        ...
        latent_samples = latent_samples.view(latent_samples.size(0), latent_samples.size(1), 1, 1)
        return self.model.decoder(latent_samples).to(self.device)
    ...
    def reconstruct(self, data, size=(8, 8), is_original=True, is_force_return=False):
        ...
        recs = recs.view(-1, *self.model.img_size).to(self.device)
    ...
    def traversals(self,
        ...
        is_force_return : bool, optional
            Force returning instead of saving the image.
        """
        n_latents = n_latents if n_latents is not None else self.model.z_dim
