...
class MinMaxScaler:
    ...
    def __init__(
        ...
        log.info('Datset Info: action min: {} and max: {}'.format(self.y_bounds[0, :], self.y_bounds[1, :]))
        self.tensor_y_bounds = torch.from_numpy(self.y_bounds).to(device)

        log.info(f'Training dataset size: target {y_data.shape}')
...
class ActionScaler:
    ...
    def __init__(self, y_data: np.ndarray, scale_data: bool, device: str):
        self.scale_data = scale_data
        self.device = device
        ...
        log.info('Datset Info: action min: {} and max: {}'.format(self.y_bounds[0, :],
                                                                  self.y_bounds[1, :]))
        self.tensor_y_bounds = torch.from_numpy(self.y_bounds).to(device)

        log.info(f'Training dataset size:  target_action_shape {y_data.shape}')
    ...
    @torch.no_grad()
    def clip_action(self, y):
        return torch.clamp(y, self.y_bounds_tensor[0, :] * 1.1, self.y_bounds_tensor[1, :] * 1.1).to(
            self.device).to(
            torch.float32)
